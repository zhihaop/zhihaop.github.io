...
3. **一些案例：**RocksDB和Cassandra 都是用 LSM。
...
4. Hybrid Log 在设计上如此快，很大一部分原因是舍弃了全序关系和一部分持久性。它不支持范围查询，并且 `Mutable` 区域没有做 WAL，也没有落盘，可能导致数据丢失，但是这种 in-place 和 out-of-place 数据结构的结合是一种很优秀的思想。  
